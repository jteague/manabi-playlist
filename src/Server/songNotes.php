...
switch($operation) {
    ...
    case "get":
        if(isset($_GET['gig_id']) && isset($_GET['user_uid'])) {
            $outp = getCreateSongNotes($conn, $_GET['gig_id'], $_GET['user_uid']);
            $conn->close();
            echo($outp);
...
function getCreateSongNotes($connection, $gig_id, $user_uid) {
    
	$countQuery = "SELECT count(id) as count FROM song_note where gig_id = $gig_id AND user_uid = '$user_uid'";
	$result = $connection->query($countQuery);
	$count = $result->fetch_array(MYSQLI_ASSOC)["count"];
	if($count < 1) {
		// we need to create the song_notes for this gig/user
		mylog("Creating SongNotes for gig ($gig_id) / user ($user_uid)...");
		$gigSongQuery = "SELECT id, gig_id, song_id FROM gig_song WHERE gig_id = $gig_id";
		$result = $connection->query($gigSongQuery);
		while($gigSong = $result->fetch_array(MYSQLI_ASSOC)) {
			$addSongNote = "INSERT INTO song_note (song_id, gig_id, user_uid, bad_horns, bad_rhythm, bad_start, bad_end) VALUES (" . $gigSong["song_id"] . ", " . $gigSong["gig_id"] . ", '" . $user_uid . "', 0, 0, 0, 0)";
			$insertResult = $connection->query($addSongNote);
		}
	}
